...
const registrationAccount = async (req: RequestData) => {
  const { files, body: data } = req as any;
  const { role, password, confirmPassword, email, ...other } = data;
  ...
  if (files && files.image) {
    profile_image = `/images/profile/${files.image[0].filename}`;
  }
